import type { TripCard } from './travel.js';
 ...
 * - finalizeTripCard：内部工具，由 chat 路由消费后合并成 'card' 事件，**不会**以 tool_start / tool_end 形式下发。
 *   保留在 ToolName 里便于类型提示，但前端不应收到它的 tool 事件。
 */
export type ToolName = 'getWeather' | 'getAttractions' | 'finalizeTripCard';
