...
所以根因是 **Vite dev server 的 HTTP proxy 转发行为**破坏了 Chrome 网络层识别 SSE 的启发式：
...
在 Vite 的 proxy `configure` 钩子里，对 SSE 响应**关闭 Nagle + 立刻 flush 头**。
...
- `flushHeaders()`：让响应头在 body 第一个 chunk 之前到达浏览器，DevTools 能更早判定响应类型。
...
3. DevTools → Network → 选中 chat 请求 → EventStream 面板应该能看到 `token` / `tool_start` / `tool_end` / `final` / `done` 等事件逐行出现。
4. 如果仍为空，可用 `curl -i -N --raw` 直接分别打 5173 和 3001，对比 chunk 边界是否合并。
